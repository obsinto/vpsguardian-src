...
(
  ...
  assert_true 'diag_field diagnosis:laravel: TITLE | grep -q "requer atenção"' "título evita diagnóstico conclusivo sem evidência suficiente"
  assert_true 'diag_field diagnosis:laravel: SUMMARY | grep -Fq "Projeto: Bug Royale\\nRecurso: bugroyale-worker\\nAmbiente: production"' "resumo organiza a identidade Coolify em linhas"
